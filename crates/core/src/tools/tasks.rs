...
mod tests {
    ...
    async fn spawn_runs_detached_with_live_log() {
        let workdir = std::env::temp_dir().to_string_lossy().into_owned();
        let t = spawn_task("echo hello-bg && echo err-line >&2", &workdir);
    ...
    async fn failing_command_reports_error() {
        let workdir = std::env::temp_dir().to_string_lossy().into_owned();
        let t = spawn_task("exit 3", &workdir);
        let done = wait_for(&t.id, 5000).await;
    ...
    async fn kill_stops_running_task() {
        let workdir = std::env::temp_dir().to_string_lossy().into_owned();
        let t = spawn_task("sleep 30", &workdir);
    ...
    async fn multibyte_log_tail_never_panics() {
        ...
        // that cannot encode ✓, which would fail the spawn instead of the
        // tail logic under test.
        let workdir = std::env::temp_dir().to_string_lossy().into_owned();
        let t = spawn_task(
            "PYTHONIOENCODING=utf-8 python3 -c \"print('\u{2713}'*50000)\"",
            &workdir,
        );
