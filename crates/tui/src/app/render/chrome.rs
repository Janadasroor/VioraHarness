...
impl App {
    ...
    pub(crate) fn draw_input(&self, frame: &mut Frame, area: Rect) {
        // Subagent view owns no input box at all — not even a read-only
        // bar. Only the main agent controls input (Esc returns). The
        // layout already collapses this row to height 0; this is a
